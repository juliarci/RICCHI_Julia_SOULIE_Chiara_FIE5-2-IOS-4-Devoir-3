...
```{r}
W_tranches_bureau %>% select(tranche, type_bureau, W_q) %>% print()
```
...
lambda_Prio_global <- sum(accueil$type_bureau == "Prio") / 10
...
# Taux d'occupation
rho_global <- lambda_Prio_global / mu_Prio
...
resultats_MM1 <- bind_rows(
  calcul_MM1(lambda_Prio_global, mu_Prio, "Global"),
...
```{r answer 2.1.2b}
# Construction de comparaison pour les graphiques 2.1.5 et 2.1.6
donnees_empiriques_prio <- data.frame(
  Periode       = c("Global", "[8h-11h20]", "[11h20-14h40]", "[14h40-18h]"),
  W_q_empirique = c(
    W_q_Prio,
    W_tranches_bureau %>% filter(tranche == "[8h;11h20]",    type_bureau == "Prio") %>% pull(W_q),
    W_tranches_bureau %>% filter(tranche == "[11h20;14h40]", type_bureau == "Prio") %>% pull(W_q),
    W_tranches_bureau %>% filter(tranche == "[14h40;18h00]", type_bureau == "Prio") %>% pull(W_q)
  ),
  L_empirique   = c(
    L_Prio,
    L_tranches_bureau %>% filter(tranche == "[8h;11h20]",    type_bureau == "Prio") %>% pull(L),
    L_tranches_bureau %>% filter(tranche == "[11h20;14h40]", type_bureau == "Prio") %>% pull(L),
    L_tranches_bureau %>% filter(tranche == "[14h40;18h00]", type_bureau == "Prio") %>% pull(L)
  ),
  L_q_empirique = c(
    L_q_Prio,
    L_tranches_bureau %>% filter(tranche == "[8h;11h20]",    type_bureau == "Prio") %>% pull(L_q),
    L_tranches_bureau %>% filter(tranche == "[11h20;14h40]", type_bureau == "Prio") %>% pull(L_q),
    L_tranches_bureau %>% filter(tranche == "[14h40;18h00]", type_bureau == "Prio") %>% pull(L_q)
  )
)

comparaison <- resultats_MM1 %>%
  select(Periode, W_q_sec, L_q, L) %>%
  left_join(donnees_empiriques_prio, by = "Periode")
```
...
resultats_MM3 <- bind_rows(
  calcul_MM3(lambda_Autres, mu_Autres, 3, "Global"),
...
```{r answer 3.1.3b}
# Construction de comparaison_MM3
donnees_empiriques_autres <- data.frame(
  Periode = c("Global", "[8h-11h20]", "[11h20-14h40]", "[14h40-18h]"),
  W_q_empirique = c(
    W_q_Autres,
    W_tranches_bureau %>% filter(tranche == "[8h;11h20]",    type_bureau == "Autres") %>% pull(W_q),
    W_tranches_bureau %>% filter(tranche == "[11h20;14h40]", type_bureau == "Autres") %>% pull(W_q),
    W_tranches_bureau %>% filter(tranche == "[14h40;18h00]", type_bureau == "Autres") %>% pull(W_q)
  ),
  L_q_empirique = c(
    L_q_Autres,
    L_tranches_bureau %>% filter(tranche == "[8h;11h20]",    type_bureau == "Autres") %>% pull(L_q),
    L_tranches_bureau %>% filter(tranche == "[11h20;14h40]", type_bureau == "Autres") %>% pull(L_q),
    L_tranches_bureau %>% filter(tranche == "[14h40;18h00]", type_bureau == "Autres") %>% pull(L_q)
  )
)
...
```{r answer 3.1.4b}
# Construction de df_pooling pour comparer M/M/3 vs 3×M/M/1
resultats_3MM1 <- bind_rows(
  calcul_MM1(lambda_Autres / 3,            mu_Autres, "Global"),
  calcul_MM1(lambda_8h_11h20_Autres / 3,   mu_Autres, "[8h-11h20]"),
  calcul_MM1(lambda_11h20_14h40_Autres / 3, mu_Autres, "[11h20-14h40]"),
  calcul_MM1(lambda_14h40_18h_Autres / 3,  mu_Autres, "[14h40-18h]")
)

df_pooling <- bind_rows(
  resultats_MM3  %>% select(Periode, W_q = W_q_sec) %>%
    mutate(Architecture = "M/M/3 (file unique)"),
  resultats_3MM1 %>% select(Periode, W_q = W_q_sec) %>%
    mutate(Architecture = "3×M/M/1 (files séparées)")
)
```
...
occupation_stats <- evenements %>%
  mutate(
    duree_sec = as.numeric(lead(time) - time, units = "secs")
  ) %>%
  ...
  summarise(
    ...
    nb_departs         = sum(type == "depart"),   # départs OBSERVÉs à ce niveau
    .groups = "drop"
  ) %>%
  mutate(
...
-   un temps d’observation relativement faible dans cet état donc peu fiable statistiquement,
...
```{r answer 4.2.3b}
# ── Fonction PC2 avec mu différenciés ────────────────────────────────────────
calcul_MM1_PC2_diff <- function(lambda1, lambda2, mu1, mu2, preemptive = FALSE) {
  S1_l <- 1 / mu1
  S2_l <- 1 / mu2
  p1_l <- lambda1 / (lambda1 + lambda2)
  p2_l <- lambda2 / (lambda1 + lambda2)
  S_m  <- p1_l * S1_l + p2_l * S2_l
  rho_l  <- (lambda1 + lambda2) * S_m
  rho1_l <- lambda1 * S1_l
  rho2_l <- lambda2 * S2_l
  if (rho_l >= 1) return(list(stable = FALSE, rho = rho_l))
  E_S2_l <- p1_l * (2 / mu1^2) + p2_l * (2 / mu2^2)
  R <- (lambda1 + lambda2) * E_S2_l / 2
  if (!preemptive) {
    Wq1_h <- R / (1 - rho1_l)
    Wq2_h <- R / ((1 - rho1_l) * (1 - rho_l))
  } else {
    Wq1_h <- rho1_l * S1_l / (1 - rho1_l)
    Wq2_h <- R / ((1 - rho1_l) * (1 - rho_l))
  }
  W1_h <- Wq1_h + S1_l
  W2_h <- Wq2_h + S2_l
  list(
    stable   = TRUE,   rho = rho_l,
    rho1     = rho1_l, rho2 = rho2_l,
    W_q1_sec = Wq1_h * 3600, W_q2_sec = Wq2_h * 3600,
    W1_sec   = W1_h  * 3600, W2_sec   = W2_h  * 3600,
    S1 = S1_l, S2 = S2_l
  )
}

# ── Calcul ───────────────────────────────────────────────────────────────────
resultats_PC2_diff <- calcul_MM1_PC2_diff(
  lambda_Prio, lambda_Autres, mu1_nouveau, mu2_nouveau, preemptive = FALSE
)

# ── Variables auxiliaires ─────────────────────────────────────────────────────
S1_nouveau      <- 3600 / mu1_nouveau
S2_nouveau      <- 3600 / mu2_nouveau
mu_moyen        <- (mu1_nouveau + mu2_nouveau) / 2
W_q_actuel_Prio   <- W_q_Prio
W_q_actuel_Autres <- W_q_Autres
W_actuel_Prio     <- W_Prio
W_actuel_Autres   <- W_Autres
...
```{r answer 4.2.4}
cat("Delta W_q Classe 1 (Q4.2 - Q4.1) =", round(diff_W_q1, 2), "sec\n")
cat("Delta W_q Classe 2 (Q4.2 - Q4.1) =", round(diff_W_q2, 2), "sec\n")
cat("Delta W  Classe 1 (Q4.2 - Q4.1) =", round(diff_W1,   2), "sec\n")
cat("Delta W  Classe 2 (Q4.2 - Q4.1) =", round(diff_W2,   2), "sec\n")
```
