...
Les bureaux d'accueil non prioritaires présentent un système globalement bien dimensionné. Le taux d'utilisation par serveur (ρ) reste très faible, oscillant entre 13% et 22% selon les tranches horaires, ce qui signifie que les agents sont largement disponibles et que le système est loin de tout risque de saturation. Cette faible charge se reflète directement dans la probabilité d'attente calculée, qui ne dépasse pas 3,2% même sur la tranche la plus chargée ([8h;11h20]), confirmant qu'un client a très peu de chances de trouver tous les serveurs occupés à son arrivée. Toutefois, la comparaison entre les temps d'attente théoriques (M/M/3) et empiriques révèle un écart notable. Si le modèle prédit des attentes inférieures à 6 secondes, les données réelles atteignent près de 37 secondes sur la tranche [8h;11h20] et environ 19 secondes en global. Cet écart suggère que les hypothèses du modèle, notamment la distribution aléatoire des arrivées et des durées de service, ne sont pas parfaitement vérifiées en pratique, possiblement en raison d'effets de groupe ou de pics d'arrivées concentrés. Enfin, l'analyse de l'effet du pooling montre sans ambiguïté l'avantage de la mutualisation des files. Une organisation en 3 files séparées (3×M/M/1) générerait des temps d'attente 30 à 50 fois supérieurs à ceux d'une file commune unique (M/M/3). Ce résultat plaide donc fortement pour le maintien d'une organisation mutualisée des bureaux non prioritaires.
...
Le graphique de comparaison des taux de service μ(n) révèle un écart structurel entre le comportement théorique et empirique. Le modèle M/M/3 suppose une montée linéaire du taux de service dès l'arrivée des premiers clients, alors qu'empiriquement le taux reste très faible pour n=1 à 3, chute quasi à zéro pour n=3, puis bondit brusquement à partir de n=4. Ce comportement suggère que les agents ne fonctionnent pas comme des serveurs strictement parallèles et indépendants, mais adoptent un mode de service différé lorsque la file est courte. Cette divergence explique directement les résultats sur le W_q. Le modèle de Markov général, qui intègre ce taux de service dépendant du niveau d'occupation, surestime largement l'attente réelle (\~38s sur toutes les tranches), tandis que le M/M/3 la sous-estime systématiquement. Les données empiriques se situent entre les deux, confirmant qu'aucun des deux modèles ne capture parfaitement la réalité.
...
La distribution du nombre de patients dans le système (M/M/1/PC=2, ρ=0.188) confirme que le système est très peu chargé : dans plus de 80% des cas, le système est vide, et la probabilité d'avoir 2 patients ou plus devient rapidement négligeable. La capacité limitée à 2 places n'est donc quasiment jamais contraignante dans les conditions actuelles, ce qui valide la pertinence de ce modèle pour ce niveau de charge. La comparaison des temps d'attente entre les trois configurations montre que le système actuel à 4 serveurs reste la référence la plus performante pour les patients prioritaires, avec un W_q quasi nul (\~1,4s), contre environ 20s pour la configuration à priorité et 23s en FIFO. Pour les non prioritaires en revanche, les trois configurations affichent des temps similaires, entre 19 et 24 secondes. L'introduction de la priorité permet ainsi de réduire l'attente des prioritaires de 14,3% par rapport au FIFO, au prix d'une légère hausse de 5,5% pour les non prioritaires, un compromis favorable si l'on cherche à protéger les patients les plus urgents. L'analyse de sensibilité au taux d'utilisation ρ illustre clairement la robustesse du système prioritaire. Pour les niveaux de charge élevés (ρ \> 0,75), l'attente des non prioritaires et du FIFO diverge très rapidement, tandis que celle des prioritaires reste bien maîtrisée. Au ρ actuel de 0,188, les trois courbes sont très proches et les temps d'attente faibles, mais cette analyse montre que si la charge venait à augmenter significativement, le système avec priorité offrirait une bien meilleure protection pour les patients urgents.
...
La comparaison des temps d'attente W_q entre les trois configurations montre que le passage à un serveur mutualisé avec taux de service différenciés (Q4.2) dégrade légèrement les performances par rapport au système actuel à 4 serveurs, avec des W_q de 30s pour les non prioritaires et 25s pour les prioritaires, contre respectivement 19s et 1,4s dans le système actuel. La configuration Q4.1 à taux identique se situe entre les deux. Cette dégradation s'explique par la réduction du nombre de serveurs, qui compense l'effet bénéfique de la différenciation des taux de service. En revanche, la comparaison des temps totaux W révèle une image radicalement différente : Q4.1 et Q4.2 affichent des temps totaux (\~125-130s) très inférieurs à ceux du système actuel (\~400-420s). Ce paradoxe apparent s'explique par la décomposition W = W_q + S : dans le système actuel, le temps de service S est beaucoup plus long car les taux de service sont plus faibles (μ/4 vs μ), ce qui domine largement le temps total malgré un W_q plus faible. L'impact de la différenciation des taux (Q4.2 - Q4.1) est quantifié sur le troisième graphique : passer à des μ différenciés augmente W_q d'environ 5 secondes pour les prioritaires et 5,5 secondes pour les non prioritaires. La différenciation ne favorise donc pas mécaniquement les prioritaires en termes d'attente en file, ce qui s'explique par le ratio actuel μ₁/μ₂ ≈ 1,001, quasi identique comme l'indique le graphique de sensibilité. C'est précisément ce que confirme l'analyse de sensibilité au ratio μ₁/μ₂ : au ratio actuel de 1,001, les deux classes sont quasi confondues et proches du minimum de W_q. Pour tirer un vrai bénéfice de la différenciation en faveur des prioritaires, il faudrait augmenter significativement μ₁ par rapport à μ₂, ce qui réduirait l'attente des prioritaires au prix d'une hausse pour les non prioritaires.
